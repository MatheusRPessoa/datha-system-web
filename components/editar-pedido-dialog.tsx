...
import { priorityLabels, FILE_FORMATS, type Order } from "@/lib/data"
...
export function EditarPedidoDialog({
  ...
  const { clients, users, updateOrder } = useStore()
  const [numeroPedido, setNumeroPedido] = useState(pedido.numero.replace(/^#/, ""))
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Erro ao salvar pedido")
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Atualize os dados do pedido {pedido.numero}.</DialogDescription>
        </DialogHeader>
